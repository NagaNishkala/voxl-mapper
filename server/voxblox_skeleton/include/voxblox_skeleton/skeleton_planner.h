 ...
 public:
  ...
  inline void setEsdfLayer(const Layer<EsdfVoxel>* esdf_layer) {
    if(esdf_layer == nullptr){
      fprintf(stderr, "esdf layer is nullptr: %s\n", __FUNCTION__);
      return;
    }
    std::cout << "Setting ESDF layer." << std::endl;
    esdf_layer_ = esdf_layer;
    if (skeleton_layer_ == nullptr) {
      std::cout << "Setting neighbor tools to not 0! "
                << esdf_layer->voxels_per_side() << std::endl;
...
bool SkeletonAStar::getPathInVoxels(
  ...
  if(voxel_path == nullptr){
    fprintf(stderr, "voxel path is nullptr:%s\n", __FUNCTION__);
    return false;
  }
  if(skeleton_layer_ == nullptr){
    fprintf(stderr, "skeleton layer is nullptr:%s\n", __FUNCTION__);
  ...
  while (!open_set.empty()) {
    ...
    if (current_voxel_offset == goal_voxel_offset) {
      ...
      std::cout << "Found solution in: " << num_iterations << std::endl;
      return true;
    }
