// #include "voxblox_skeleton/io/skeleton_io.h" //TODO: needs rewrite - proto
...
SkeletonGenerator::SkeletonGenerator(Layer<EsdfVoxel>* esdf_layer)
  ...
  if(esdf_layer == nullptr){
    throw std::runtime_error(std::string("esdf layer was set to null for planner"));
  }
}

void SkeletonGenerator::setEsdfLayer(Layer<EsdfVoxel>* esdf_layer) {
  if(esdf_layer == nullptr){
    fprintf(stderr, "esdf_layer is null: %s\n", __FUNCTION__);
    return;
  }
  ...
  if(voxel_size_ != skeleton_layer_->voxel_size()){
    fprintf(stderr, "voxel size and skeleton layer voxel size do not match: %s\n", __FUNCTION__);
    return;
  }
  if(voxels_per_side_ != skeleton_layer_->voxels_per_side()){
    fprintf(stderr, "voxels per side and skeleton layer per side do not match: %s\n", __FUNCTION__);
    return;
  }
}
...
void SkeletonGenerator::updateSkeletonFromLayer() {
  if(skeleton_layer_ == nullptr){
    fprintf(stderr, "skeleton layer is nullptr: %s\n", __FUNCTION__);
    return;
  }
  ...
  for (const BlockIndex& block_index : blocks) {
    ...
    // per this issue - https://github.com/ethz-asl/mav_voxblox_planning/issues/33
...
void SkeletonGenerator::generateSkeleton() {
  ...
  for (const BlockIndex& block_index : blocks) {
    ...
    for (size_t lin_index = 0u; lin_index < num_voxels_per_block; ++lin_index) {
      ...
      if(esdf_voxel.distance < min_gvd_distance_){
        gvd_distance++;
        // printf("too close\n");
        // continue;
      }
      ...
      if (parent_dir.norm() < 1e-6f) {
       // printf("CONTINUING AGAIN\n");
        continue;
      }
      ...
      for (size_t i = 0; i < neighbors.size(); ++i) {
        ...
        if(!neighbor_block->isValidVoxelIndex(neighbor_voxel_index)){
          fprintf(stderr, "neighbor block is not a valid voxel index: %s\n", __FUNCTION__);
          return;
        }
        std::cout << "Neigbor voxel index: " << neighbor_voxel_index.transpose() << std::endl;
        ...
        if (relative_direction.norm() < 1e-6f) {
        ...
        if (acos(dot_prod) >= (double)min_separation_angle_) {
  ...
  std::cout
      ...
      << " edges: " << skeleton_.getEdgePoints().size() << std::endl;

    printf("Failures: gvd-%d, fixed-%d, observed-%d\n", gvd_distance, fixed, not_observed);
  if (generate_by_layer_neighbors_) {
...
void SkeletonGenerator::generateEdgesByLayerNeighbors() {
  ...
  for (const SkeletonPoint& point : skeleton_points) {
    ...
    if(block_ptr == nullptr){
      fprintf(stderr, "block ptr is null: %s\n", __FUNCTION__);
      return;
    }
    VoxelIndex voxel_index =
...
void SkeletonGenerator::generateSparseGraph() {
  ...
  const float grid_size_inv = 1.0f / voxel_size_;
  ...
  std::cout << "[Sparse Graph] Vertices: " << graph_.getVertexMap().size()
            << " Edges: " << graph_.getEdgeMap().size() << std::endl;
}
...
void SkeletonGenerator::pruneDiagramVertices() {
  ...
  std::cout << "[Prune] Number of vertices before prune: " << num_vertices
            << " Number of deleted vertices: " << deletion_index.size() << std::endl;
...
void SkeletonGenerator::splitSpecificEdges(
  ...
  for (size_t i = 0; i < edge_ids.size(); ++i) {
    ...
    float max_d = getMaxEdgeDistanceFromStraightLine(
  ...
  std::cout << "[Split Edges] Num vertices added: " << num_vertices_added << std::endl;
}
...
float SkeletonGenerator::getMaxEdgeDistanceFromStraightLine(
  ...
  if(coordinate_path == nullptr){
    fprintf(stderr, "coord path is nullptr: %s\n", __FUNCTION__);
    return -1.0f;
  }
  if(max_index == nullptr){
    fprintf(stderr, "max index is nullptr: %s\n", __FUNCTION__);
    return -1.0f;
  }
  *max_index = 0;
  bool success =
...
void SkeletonGenerator::repairGraph() {
  ...
  std::cout << "[Subgraph] Number of disconnected subgraphs: "
            << subgraph_vertex_examples.size() << std::endl;
  ...
  std::cout << "[Subgraph] Trying to check if we need to split "
            << new_edge_ids.size() << " new edges." << std::endl;
  ...
  std::cout << "[Subgraph] Final number of disconnected subgraphs: "
            << unique_subgraphs.size() << std::endl;
}
...
void SkeletonGenerator::tryToFindEdgesInCoordinatePath(
  ...
  if(new_edge_ids == nullptr){
    fprintf(stderr, "new edge ids is nullptr: %s\n", __FUNCTION__);
    return;
  }
...
void SkeletonGenerator::simplifyVertices() {
  ...
  for (const int64_t vertex_id : vertex_ids) {
    ...
    if (vertex.edge_list.size() == 2) {
      ...
      size_t max_index;
      float max_d = getMaxEdgeDistanceOnPath(
  ...
  std::cout << "[Simplify Vertices] Vertex removals: " << vertices_removed
            << " / " << vertex_removal_candidates
            << ", Edges added: " << edges_added << std::endl;
}
...
void SkeletonGenerator::reconnectSubgraphsAlongEsdf() {
  ...
  for (const int64_t vertex_id : vertex_ids) {
    ...
    size_t num_results = kd_tree.knnSearch(vertex.point.data(), kNumNeighbors,
                                           &ret_index[0], &out_dist_sqr[0]);
    for (size_t i = 0; i < num_results; i++) {
      if(!graph_.hasVertex(ret_index[i])){
        fprintf(stderr, "graph missing vertext: %s\n", __FUNCTION__);
        return;
      }
      const SkeletonVertex& neighbor_vertex = graph_.getVertex(ret_index[i]);
  ...
  std::cout << "[Subgraph] Potential edge candidates: "
            << potential_edge_candidates
            << " diagram candidates: " << diagram_candidates << std::endl;
...
// TODO: write i/o stuff eventually
