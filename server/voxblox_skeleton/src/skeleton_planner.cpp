...
SkeletonAStar::SkeletonAStar(const Layer<SkeletonVoxel>* skeleton_layer)
  ...
  if(skeleton_layer_ == nullptr){
    throw std::runtime_error(std::string("skeleton layer is nullptr"));
  }
}
...
bool SkeletonAStar::getPathInEsdf(const Point& start_position,
  ...
  if(coordinate_path == nullptr){
    fprintf(stderr, "coord path is a nullptr: %s\n", __FUNCTION__);
    return false;
  }
  if(esdf_layer_ == nullptr){
    fprintf(stderr, "esdf layer is a nullptr: %s\n", __FUNCTION__);
    return false;
  }
...
bool SkeletonAStar::getPathOnDiagram(
  ...
  if(skeleton_layer_ == nullptr){
    fprintf(stderr, "skeleton layer is a nullptr: %s\n", __FUNCTION__);
    return false;
  }
...
void SkeletonAStar::voxelPathToCoordinatePath(
    const Point& start_location, float voxel_size,
...
void SkeletonAStar::getSolutionPath(
  ...
  if(voxel_path == nullptr){
    fprintf(stderr, "voxel_path is a nullptr: %s\n", __FUNCTION__);
    return;
  }
  voxel_path->clear();
...
float SkeletonAStar::estimateCostToGoal(
  ...
  return (goal_voxel_offset.cast<float>() -
          voxel_offset.cast<float>())
      .norm();
}
...
bool SkeletonAStar::getPathToNearestDiagramPt(
    ...
    AlignedVector<Eigen::Vector3i>* voxel_path) const {
  if(voxel_path == nullptr){
    fprintf(stderr, "voxel path is a nullptr: %s\n", __FUNCTION__);
    return false;
  }
  if(skeleton_layer_ == nullptr){
    fprintf(stderr, "skeleton layer is a nullptr: %s\n", __FUNCTION__);
