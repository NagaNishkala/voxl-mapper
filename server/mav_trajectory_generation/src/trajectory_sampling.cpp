...
bool sampleTrajectoryAtTime(const Trajectory& trajectory, double sample_time,
    ...
    if (state == nullptr) {
        fprintf(stderr, "state is a nullptr: %s\n", __FUNCTION__);
        return false;
    }
    if (sample_time < trajectory.getMinTime() ||
        ...
        std::cout << "Sample time should be within [" << trajectory.getMinTime()
                  << " " << trajectory.getMaxTime() << "] but is " << sample_time << std::endl;
        return false;
    }

    if (trajectory.D() < 3) {
        std::cout << "Dimension has to be at least 3, but is " << trajectory.D() << std::endl;
        return false;
    }
...
bool sampleWholeTrajectory(const Trajectory& trajectory,
    ...
    // fprintf(stderr, "now sampling whole trajectory\n");
...
bool sampleSegmentAtTime(const Segment& segment, double sample_time,
    ...
    if (sample_time < 0.0 || sample_time > segment.getTime()) {
        std::cout << "Sample time should be within [" << 0.0 << " "
                   << segment.getTime() << "] but is " << sample_time << std::endl;
        return false;
    }
...
bool sampleFlatStateAtTime(const T& type, double sample_time,
                           mav_msgs::EigenTrajectoryPoint* state) {
    if (type.D() < 3) {
        std::cout << "Dimension has to be 3, 4, or 6 but is " << type.D() << std::endl;
        return false;
    }
