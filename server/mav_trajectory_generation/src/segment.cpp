...
Polynomial& Segment::operator[](size_t idx) {
    if (idx >= static_cast<size_t>(D_)) {
        throw std::runtime_error(std::string("idx is too large"));
    }
...
void printSegment(std::ostream& stream, const Segment& s, int derivative) {
    if (derivative < 0 && derivative >= s.N()) {
        fprintf(stderr, "derivative is not in bounds: %s\n", __FUNCTION__);
        return;
    }
...
bool Segment::computeMinMaxMagnitudeCandidateTimes(
    ...
    if (candidate_times == nullptr) {
        fprintf(stderr, "candidate times is a nullptr: %s\n", __FUNCTION__);
        return false;
    }
    candidate_times->clear();
    // Compute magnitude derivative roots.
    if (dimensions.empty()) {
        std::cout << "No dimensions specified." << std::endl;
        return false;
        ...
        for (int dim : dimensions) {
            if (dim < 0 || dim >= D_) {
                std::cout << "Specified dimensions " << dim
...
bool Segment::selectMinMaxMagnitudeFromCandidates(
    __attribute__((unused)) int derivative, double t_start, double t_end,
    __attribute__((unused)) const std::vector<int>& dimensions, const std::vector<Extremum>& candidates,
    ...
    if (t_start > t_end) {
        std::cout << "t_start is greater than t_end." << std::endl;;
        return false;
    }
...
bool Segment::getSegmentWithSingleDimension(int dimension,
    ...
    if (dimension < 0 || dimension >= D_) {
        std::cout << "You shan't ask for a dimension that does not exist in the segment." << std::endl;
        return false;
    }
...
bool Segment::offsetSegment(const Eigen::VectorXd& A_r_B) {
    if (A_r_B.size() < std::min(D_, 3)) {
        std::cout << "Offset vector size smaller than segment dimension." << std::endl;
        return false;
    }

    // Only translate the first three dimensions.
    for (size_t i = 0; i < (size_t)std::min(D_, 3); ++i) {
