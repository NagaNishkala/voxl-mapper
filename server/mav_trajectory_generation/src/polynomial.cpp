...
bool Polynomial::selectMinMaxCandidatesFromRoots(
    ...
    if(candidates == nullptr){
        fprintf(stderr, "candidates in a nullptr: %s\n", __FUNCTION__);
    }
    if (t_start > t_end) {
...
bool Polynomial::computeMinMaxCandidates(
    ...
    if (N_ - derivative - 1 < 0) {
        std::cout << "N - derivative - 1 has to be at least 0." << std::endl;
        return false;
    }
    Eigen::VectorXcd roots;
    bool success = getRoots(derivative + 1, &roots);
    if (!success) {
        std::cout << "Couldn't find roots, polynomial may be constant." << std::endl;
    }
...
bool Polynomial::selectMinMaxFromCandidates(
    ...
    if (candidates.empty()) {
        std::cout << "Cannot find extrema from an empty candidates vector." << std::endl;
        return false;
    }
...
bool Polynomial::getPolynomialWithAppendedCoefficients(
    ...
    if (new_N == N_) {
        *new_polynomial = *this;
        ...
        std::cout << "You shan't decrease the number of coefficients." << std::endl;
        *new_polynomial = *this;
        return false;
    } else {
