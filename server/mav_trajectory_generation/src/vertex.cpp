...
Vertex::Vector createRandomVertices(int maximum_derivative, size_t n_segments,
                                    ...
                                    size_t seed) {
    if(static_cast<int>(n_segments) < 1){
        throw std::runtime_error(std::string("num segments is < 1"));
    }
    if(pos_min.size() != pos_max.size()){
        throw std::runtime_error(std::string("pos min and max are different sizes"));
    }
    if((pos_max - pos_min).norm() <= 0.2){
        throw std::runtime_error(std::string("pos max and min norm is too small"));
    }
    if(maximum_derivative <= 0){
        throw std::runtime_error(std::string("max derivative is <= 0"));
    }
...
void Vertex::addConstraint(int derivative_order,
    ...
    if(constraint.rows() != static_cast<long>(D_)){
        fprintf(stderr, "constraints rows are the wrong size: %s\n", __FUNCTION__);
        return;
    }
...
bool Vertex::getConstraint(int derivative_order, Eigen::VectorXd* value) const {
    if(value == nullptr){
        fprintf(stderr, "value is nullptr: %s\n", __FUNCTION__);
        return false;
    }
...
bool Vertex::getSubdimension(const std::vector<size_t>& subdimensions,
    ...
    if(subvertex == nullptr){
        fprintf(stderr, "subvertex is nullptr: %s\n", __FUNCTION__);
        return false;
    }
    *subvertex = Vertex(subdimensions.size());
    ...
    for (size_t subdimension : subdimensions)
        if (subdimension >= (size_t)D_) return false;
...
std::vector<double> estimateSegmentTimesVelocityRamp(
    ...
    __attribute__((unused)) double time_factor) {
    if(vertices.size() < 2){
        throw std::runtime_error(std::string("vertices size is < 2"));
    }
