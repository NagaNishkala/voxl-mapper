...
Eigen::VectorXd Trajectory::evaluate(double t, int derivative_order) const {
    ...
    if (t > accumulated_time) {
        std::cout << "Time out of range of the trajectory!" << std::endl;
...
void Trajectory::evaluateRange(double t_start, double t_end, double dt,
    ...
    if (t_start > accumulated_time) {
        std::cerr << "Start time out of range of the trajectory!" << std::endl;
        return;
    }
...
Trajectory Trajectory::getTrajectoryWithSingleDimension(int dimension) const {
    if (dimension > D_) {
        throw std::runtime_error(std::string("dimension is too large"));
    }
...
bool Trajectory::getTrajectoryWithAppendedDimension(
    ...
    if(static_cast<int>(segments_.size()) != trajectory_to_append.K()){
...
bool Trajectory::computeMinMaxMagnitude(int derivative,
    ...
    if(minimum==nullptr){
      fprintf(stderr, "minimum is a nullptr: %s\n", __FUNCTION__);
      return false;
    }
    if(maximum==nullptr){
      fprintf(stderr, "maximum is a nullptr: %s\n", __FUNCTION__);
      return false;
    }
...
bool Trajectory::addTrajectories(const std::vector<Trajectory>& trajectories,
    ...
    if(merged == nullptr){
        fprintf(stderr, "merged is a nullptr: %s\n", __FUNCTION__);
        return false;
    }
    merged->clear();
    *merged = *this;
    ...
    for (const Trajectory& t : trajectories) {
        ...
        if (t.D() != D_ || t.N() != N_) {
            std::cout << "Dimension to append: " << t.D()
                         << " this dimension: " << D_ << std::endl;
            std::cout << "Number of coefficients to append: " << t.N()
                         << " this number of coefficients: " << N_ << std::endl;
            return false;
        }
        // Add segments.
        Segment::Vector segments;
...
bool Trajectory::offsetTrajectory(const Eigen::VectorXd& A_r_B) {
    if (A_r_B.size() < std::min(D_, 3)) {
        std::cout << "Offset vector size smaller than trajectory dimension." << std::endl;
        return false;
    }
...
Vertex Trajectory::getVertexAtTime(double t, int max_derivative_order) const {
    Vertex v(D_);
    for (size_t i = 0; i <= (size_t)max_derivative_order; i++) {
...
bool Trajectory::getVertices(int max_derivative_order_pos,
    ...
    if(pos_vertices==nullptr){
      fprintf(stderr, "pos_vertices is a nullptr: %s\n", __FUNCTION__);
      return false;
    }
    if(yaw_vertices==nullptr){
      fprintf(stderr, "yaw_vertices is a nullptr: %s\n", __FUNCTION__);
      return false;
    }
...
bool Trajectory::getVertices(int max_derivative_order,
    ...
    if(vertices==nullptr){
      fprintf(stderr, "vertices is a nullptr: %s\n", __FUNCTION__);
      return false;
    }
