...
int PolynomialOptimizationNonLinear<_N>::optimizeTime() {
    ...
    try {
        ...
        std::cout << "error while setting up nlopt: " << e.what() << std::endl;
    ...
    try {
        ...
        std::cout << "error while running nlopt: " << e.what() << std::endl;
        return nlopt::FAILURE;
    }
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeMellingerOuterLoop() {
    ...
    try {
        ...
        std::cout << "error while running nlopt. This likely means the optimization aborted early."
                  << std::endl;
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeAndFreeConstraints() {
    ...
    if (free_constraints.size() == 0 || free_constraints.front().size() == 0) {
        std::cout << "No free derivative variables, same as time-only optimization." << std::endl;
    }

    const size_t n_optimization_variables =
        n_segments + free_constraints.size() * free_constraints.front().size();

    if (n_optimization_variables <= 0u) {
        fprintf(stderr, "n_optimization_variables is <= 0: %s\n", __FUNCTION__);
        return nlopt::FAILURE;
    }
    ...
    if(lower_bounds.size() != upper_bounds.size()){
        fprintf(stderr, "bounds/solution/step sizes do not match: %s\n", __FUNCTION__);
        return nlopt::FAILURE;
    }
    if(initial_solution.size() != lower_bounds.size()){
        fprintf(stderr, "bounds/solution/step sizes do not match: %s\n", __FUNCTION__);
        return nlopt::FAILURE;
    }
    if(initial_solution.size() != initial_step.size()){
        fprintf(stderr, "bounds/solution/step sizes do not match: %s\n", __FUNCTION__);
        return nlopt::FAILURE;
    }
    if(initial_solution.size() != n_optimization_variables){
        fprintf(stderr, "bounds/solution/step sizes do not match: %s\n", __FUNCTION__);
        return nlopt::FAILURE;
    }
...
bool PolynomialOptimizationNonLinear<_N>::addMaximumMagnitudeConstraint(
    ...
    if(derivative < 0){
        fprintf(stderr, "derivative < 0: %s\n", __FUNCTION__);
        return false;
    }
    if(maximum_value < 0.0){
        fprintf(stderr, "max value < 0: %s\n", __FUNCTION__);
        return false;
    }
    ...
    if (!optimization_parameters_.use_soft_constraints) {
        try {
            ...
            std::cout << "ERROR while setting inequality constraint " << e.what()
                       << std::endl;
            return false;
        }
    }
...
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTime(
    ...
    void* data) {
    if(gradient.empty()){
        std::cout << "computing gradient not possible, choose a gradient free method" << __FUNCTION__ << std::endl;
        return -1.0;
    }

    if(data == nullptr){
        fprintf(stderr, "data is a nullptr: %s\n", __FUNCTION__);
        return -1.0;
    }
    ...
    if(segment_times.size() != optimization_data->poly_opt_.getNumberSegments()){
        fprintf(stderr, "num segments does not match: %s\n", __FUNCTION__);
        return -1.0;
    }
...
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTimeAndConstraints(
    ...
    if(x.size() != n_segments + n_free_constraints * dim){
        fprintf(stderr, "sizes do not match: %s\n", __FUNCTION__);
        return -1.0;
    }
...
double PolynomialOptimizationNonLinear<_N>::evaluateMaximumMagnitudeConstraint(
    __attribute__((unused)) const std::vector<double>& segment_times, std::vector<double>& gradient,
    void* data) {
    if(gradient.empty()){
        std::cout << "computing gradient not possible, choose a gradient free method" << __FUNCTION__ << std::endl;
        return -1.0;
...
PolynomialOptimizationNonLinear<_N>::evaluateMaximumMagnitudeAsSoftConstraint(
    __attribute__((unused)) const std::vector<std::shared_ptr<ConstraintData> >& inequality_constraints,
...
void PolynomialOptimizationNonLinear<_N>::
    ...
    if(lower_bounds == nullptr){
        fprintf(stderr, "bounds are nullptr: %s\n", __FUNCTION__);
        return;
    }
    ...
    if(lower_bounds->empty()){
        std::cout << "Lower bounds not empty!";
        return;
    }
    if(upper_bounds->empty()){
        std::cout << "Upper bounds not empty!";
        return;
    }
