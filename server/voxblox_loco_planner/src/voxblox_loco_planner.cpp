...
VoxbloxLocoPlanner::VoxbloxLocoPlanner(LocoPlannerParams _loco_params, ShotgunParameters _shotgun_params,
                                       PhysicalConstraints _phys_constraints){
    verbose_ = _loco_params.verbose;
    visualize_ = false;
    num_segments_ = _loco_params.num_segments;
    num_random_restarts_ = _loco_params.num_random_restarts;
    random_restart_magnitude_ = _loco_params.random_restart_magnitude;
    planning_horizon_m_ = _loco_params.planning_horizon_m;
    use_shotgun_ = _loco_params.use_shotgun;
    use_shotgun_path_ = _loco_params.use_shotgun_path;
    //loco_(kD);

    constraints_ = _phys_constraints;
    shotgun_ = _shotgun_params;

    loco_.setRobotRadius(constraints_.robot_radius);
    loco_.setEpsilon(constraints_.robot_radius + (double)_loco_params.loco_epsilon_inflation);
...
void VoxbloxLocoPlanner::setEsdfMap(const std::shared_ptr<voxblox::EsdfMap>& esdf_map) {
    if(esdf_map == nullptr){
        fprintf(stderr, "esdf_map is a nullptr: %s\n", __FUNCTION__);
        return;
    }
    esdf_map_ = esdf_map;
...
double VoxbloxLocoPlanner::getMapDistance(const Eigen::Vector3d& position) const {
    double distance = 0.0;
...
double VoxbloxLocoPlanner::getMapDistanceAndGradientVector(const Eigen::VectorXd& position, Eigen::VectorXd* gradient) const {
    if(position.size() != 3){
        fprintf(stderr, "position is not equal to 3: %s\n", __FUNCTION__);
    }
    if (gradient == nullptr) {
...
double VoxbloxLocoPlanner::getMapDistanceAndGradient(const Eigen::Vector3d& position, Eigen::Vector3d* gradient) const {
    double distance = 0.0;
    const bool kInterpolate = false;
    if (!esdf_map_->getDistanceAndGradientAtPosition(position, kInterpolate, &distance, gradient)) {
        return 0.0;
    }
    return distance;
}
...
bool VoxbloxLocoPlanner::isPathCollisionFree(const mav_msgs::EigenTrajectoryPointVector& path) const {
...
bool VoxbloxLocoPlanner::isPathFeasible(const mav_msgs::EigenTrajectoryPointVector& path) const {
...
bool VoxbloxLocoPlanner::getTrajectoryBetweenWaypoints(const mav_msgs::EigenTrajectoryPoint& start,
    ...
    std::cout <<"[Voxblox Loco Planner] Start: " << start.position_W.transpose() << " goal: " << goal.position_W.transpose() << std::endl;
    ...
    if (total_time < kMinTime) {
        printf("start and goal are basically the same thing\n");
        return false;
    }
    ...
    if (!initial_path.empty() && (unsigned int)num_segments_ == initial_path.size() - 1) {
    ...
    if (verbose_) {
        printf("[Voxblox Loco Planner] Found solution (%d) after %d restarts.\n", success, i);
    }
    return success;
}

bool VoxbloxLocoPlanner::getInitialTrajectory(const mav_msgs::EigenTrajectoryPoint::Vector& waypoints, double total_time,
...
bool VoxbloxLocoPlanner::getTrajectoryTowardGoal(
    ...
    if(trajectory == nullptr){
        fprintf(stderr, "trajectory is a nullptr: %s\n", __FUNCTION__);
        return false;
    }
    trajectory->clear();
    ...
    if ((goal_point.position_W - start_point.position_W).norm() <
        kGoalReachedRange) {
        //if (verbose_) {
            printf("[Voxblox Loco Planner] Goal already reached!\n");
        //}
        return true;
    }
    ...
    if (use_shotgun_) {
       ...
       // if (verbose_) {
            printf("[Shotgun] Found (%d) intermediate goal at %f %f %f\n", goal_found,
        ...
        //     kGoalReachedRange) {
        //     //if (verbose_) {
        //         printf("[Voxblox Loco Planner] Intermediate goal already reached!\n");
        //    // }
        //     return true;
        // }
...
bool VoxbloxLocoPlanner::findIntermediateGoalShotgun(
    ...
    if(goal_out == nullptr){
        fprintf(stderr, "goal out is nullptr: %s\n", __FUNCTION__);
        return false;
    }
...
bool VoxbloxLocoPlanner::getNearestFreeSpaceToPoint(
    ...
    if(esdf_map_ == nullptr){
        fprintf(stderr, "esdf map is a null ptr: %s\n", __FUNCTION__);
        return false;
    }
