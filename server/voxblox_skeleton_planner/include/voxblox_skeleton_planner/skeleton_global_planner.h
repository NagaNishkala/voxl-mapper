...
class SkeletonGlobalPlanner {
 public:
  ...
  SkeletonGlobalPlanner();
  SkeletonGlobalPlanner(bool visualize, voxblox::EsdfMap::Ptr esdf_map_ptr, PhysicalConstraints phys_const);
  virtual ~SkeletonGlobalPlanner() {}

  void generateSparseGraph();

  bool plannerServiceCallback(mav_msgs::EigenTrajectoryPoint start_pose, mav_msgs::EigenTrajectoryPoint goal_pose, mav_msgs::EigenTrajectoryPointVector loco_path);
