...
bool EsdfPathShortener::shortenPath(
    ...
    if (shortened_path == nullptr) {
        fprintf(stderr, "shortened_path is a nullptr: %s\n", __FUNCTION__);
        return false;
    }
...
bool EsdfPathShortener::isLineInCollision(const Eigen::Vector3d& start,
    ...
    if(voxel_size_ <= 0.0){
        fprintf(stderr, "voxel size is invalid: %s\n", __FUNCTION__);
        return false;
    }
    ...
    while (distance_so_far <= distance) {
        voxblox::EsdfVoxel* esdf_voxel = esdf_layer_->getVoxelPtrByCoordinates(
            current_position.cast<float>());
        if (esdf_voxel == nullptr) {
            return true;
        }
        if ((double)esdf_voxel->distance < constraints_.robot_radius) {
            return true;
        }

        double step_size =
            std::max(voxel_size_, (double)esdf_voxel->distance - constraints_.robot_radius);
